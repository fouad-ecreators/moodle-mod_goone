...
final class create_course_form extends \totara_form\form {
    ...
    public function definition() {
        ...
        $settings = $this->model->add(new section('settings', 'Course settings'));
        $settings->set_collapsible(false);

        if ($this->parameters['totalselected'] == 1) {
            $multiactivitylabel = 'Create a new multi-activity course for the selected content item';
            $singleactivitylabel = 'Create a new single activity course for the selected content item';
        } else {
            $multiactivitylabel = 'Create a new multi-activity course for the selected content items';
            $singleactivitylabel = 'Create a new single activity course for each selected content item';
        }
        $options = [
        ...
        if ($this->parameters['totalselected'] == 1 || $this->parameters['create'] == self::CREATE_COURSE_MULTI_ACTIVITY) {
            ...
            $submitlabel = 'Create and view course';
    ...
    protected function validation(array $data, array $files) {
        ...
        if ($data['create'] == self::CREATE_COURSE_MULTI_ACTIVITY && count($data['selection']) > 1) {
            ...
            if ($data[$elementname] == 0) {
                $errors[$elementname] = get_string('misssingcoursecategory', 'totara_contentmarketplace');
            }
        } else {
    ...
    static function get_existing_course($shortname) {
        global $DB;
