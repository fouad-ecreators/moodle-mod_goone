...
final class oauth {
    ...
    const ENDPOINT = 'https://auth.goone.com';
    ...
    public function __construct($config, $curl = null) {
        $this->config = $config;
        $this->client = new rest_client(self::ENDPOINT, $curl);
    }

    static public function get_authorize_url($redirect_uri, $state) {
        $url = self::ENDPOINT . '/oauth/authorize';
        if (defined('BEHAT_SITE_RUNNING') || defined('goone_MOCK_API_ENDPOINTS')) {
     ...
     */
    static public function move_config_from_session_to_db() {
    ...
    static public function have_config_in_session() {
        $sessionstorage = new config_session_storage();
